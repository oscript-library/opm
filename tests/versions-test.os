...
Функция ПолучитьСписокТестов(Знач Тестирование) Экспорт
	...
	СписокТестов.Добавить("ТестДолжен_ОтличитьНомерВерсииОтВыраженияИТега");
...
// Номер версии — то, чем может называться артефакт на хабе, который ничего не разрешает.
Процедура ТестДолжен_ОтличитьНомерВерсииОтВыраженияИТега() Экспорт

	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.2.3"), "релиз").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.4"), "легаси-двойка").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.2.3.4"), "легаси-четвёрка").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.3.0-rc.1"), "пререлиз").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.0.0+build"), "сборка").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("SNAPSHOT"), "снапшот").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("snapshot"), "снапшот строчными").Равно(Истина);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("1.x"), "маска версии").Равно(Ложь);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("^1.4"), "каретный диапазон").Равно(Ложь);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("~1.4"), "тильдовый диапазон").Равно(Ложь);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии(">=1.0"), "диапазон со сравнением").Равно(Ложь);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии("stable"), "тег").Равно(Ложь);
	Ожидаем.Что(РаботаСВерсиями.ЭтоТочныйНомерВерсии(""), "пустая спецификация").Равно(Ложь);
