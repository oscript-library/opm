...
Функция ОпределитьТекущийКоммит(Знач ГитРепозиторий)
...
// TODO: перенести в gitrunner
Функция ПолучитьВнешнююСсылку(
	Знач ГитРепозиторий,
	Знач ИмяВнешнейСсылки = "origin",
	Знач Режим = "fetch")

	ТаблицаВнешнихСсылок = ГитРепозиторий.ПолучитьСписокВнешнихРепозиториев();
	ЧтоИщем = Новый Структура("Имя, Режим", ИмяВнешнейСсылки, Режим);
	НужныеСсылки = ТаблицаВнешнихСсылок.НайтиСтроки(ЧтоИщем);
	
	Если НужныеСсылки.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон("У репозитория нет ссылки на %1 (%2)", ИмяВнешнейСсылки, Режим);
	КонецЕсли;
	
	Возврат НужныеСсылки.Получить(0).Адрес;

КонецФункции

Процедура ДобавитьОтметкуГит(Знач Описание, Знач КаталогРепозитория)
	Перем Гит, Параметры, Адрес, Коммит;

	Гит = Новый ГитРепозиторий;
	Гит.УстановитьРабочийКаталог(КаталогРепозитория);
	Адрес = ПолучитьВнешнююСсылку(Гит);
	Коммит = ОпределитьТекущийКоммит(Гит);
	
	Описание.ОтметкаГит(Адрес, Коммит);

КонецПроцедуры

Процедура СобратьПакет(Знач КаталогИсходников,
                       Знач ФайлМанифеста,
                       Знач ВыходнойКаталог,
                       Знач НеДобавлятьОтметкуГит = Ложь) Экспорт
	...
	Попытка
		...
		Если Не НеДобавлятьОтметкуГит Тогда

			ДобавитьОтметкуГит(Манифест, РабочийКаталог);
