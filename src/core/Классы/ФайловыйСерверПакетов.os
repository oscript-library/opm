...
//	Спецификация - Строка, Неопределено - строка после @. Каталог разрешать её не умеет,
//		поэтому названная спецификация запрещает подбор версии: нет файла — 404,
//		а не молчаливая подмена старшей версией каталога
//
...
Функция ПолучитьРесурс(Знач ИмяРесурса, Знач Спецификация = Неопределено) Экспорт
	...
	ПодборВерсииРазрешён = (Спецификация = Неопределено);

	Попытка

		Возврат Новый ФайловыйСерверОтвет(ПутьКРесурсу, ПодборВерсииРазрешён);
