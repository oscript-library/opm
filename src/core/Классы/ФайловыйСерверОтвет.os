...
// ПодборВерсииРазрешён - Ложь запрещает подставлять старшую версию каталога вместо
// ненайденного файла. Так отвечают на запрос с названной спецификацией: подобрать её
// каталогу нечем, и подстановка была бы молчаливой подменой версии.
Процедура ПриСозданииОбъекта(Знач ПутьКФайлу, Знач ПодборВерсииРазрешён = Истина)
	...
	Иначе
		...
		Если ПодборВерсииРазрешён И СтрЗаканчиваетсяНа(ПутьКФайлу, ".ospx") Тогда
