...
Перем СерверТолькоДляКорневогоПакета;
...
Процедура ПриСозданииОбъекта(Знач ВходящийРежимУстановкиПакетов = Неопределено,
	...
	СерверТолькоДляКорневогоПакета = Ложь;
КонецПроцедуры

// Ограничивает закрепление сервера пакетов корневым пакетом: зависимости при этом
// разрешаются по всем описанным серверам. Нужно адресу пула в командной строке —
// в чужом пуле зависимостей может не быть.
//
// Параметры:
//  ПСерверТолькоДляКорневогоПакета - Булево - Истина ограничивает закрепление.
//
Процедура ЗакрепитьСерверЗаКорневымПакетом(Знач ПСерверТолькоДляКорневогоПакета) Экспорт
	СерверТолькоДляКорневогоПакета = ПСерверТолькоДляКорневогоПакета;
КонецПроцедуры
...
Процедура УстановитьПакетПоИмениИВерсии(Знач ИмяПакета, Знач ВерсияПакета, ЗНач ЭтоЗависимыйПакет = Ложь, Знач УровеньЗависимости = 0) Экспорт
	...
	СерверЗапроса = ИмяСервера;
	Если СерверТолькоДляКорневогоПакета И УровеньЗависимости > 0 Тогда
		СерверЗапроса = "";
	КонецЕсли;

	ФайлПакета = РаботаСПакетами.ПолучитьПакет(ИмяПакета, ВерсияПакета, , СерверЗапроса);
