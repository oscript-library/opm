...
// Может ли строка быть номером опубликованной версии, то есть именем артефакта
// на хабе, который ничего не разрешает. Номер — цифры, разделённые точками, с
// необязательным суффиксом после дефиса или плюса; плюс особая версия SNAPSHOT.
//
// Параметры:
//  Значение - Строка - строка после символа @.
//
// Возвращаемое значение:
//  Булево - Истина, если строка может оказаться номером версии.
//
Функция ЭтоТочныйНомерВерсии(Знач Значение) Экспорт
	...
	Верхняя = ВРег(Строка_);
	Если Верхняя = "SNAPSHOT" ИЛИ СтрЗаканчиваетсяНа(Верхняя, "-SNAPSHOT") Тогда
		Возврат Истина;
	КонецЕсли;

	Возврат ЭтоЦепочкаЧисел(ЧастьДоСуффикса(Строка_));

КонецФункции

// Суффикс версии начинается с дефиса (пререлиз) либо плюса (сборка).
Функция ЧастьДоСуффикса(Знач Значение)

	Возврат СтрРазделить(СтрРазделить(Значение, "-")[0], "+")[0];

КонецФункции

Функция ЭтоЦепочкаЧисел(Знач Значение)

	Составляющие = СтрРазделить(Значение, ".", Истина);
	...
	Для Каждого Составляющая Из Составляющие Цикл
		...
		Если ПустаяСтрока(Составляющая) Тогда
			Возврат Ложь;
		КонецЕсли;

		Для Индекс = 1 По СтрДлина(Составляющая) Цикл
			Если СтрНайти("0123456789", Сред(Составляющая, Индекс, 1)) = 0 Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
