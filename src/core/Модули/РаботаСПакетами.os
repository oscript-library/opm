...
Процедура СоздатьКонфигурационныеФайлыОСкрипт(Знач КаталогУстановки, Знач ОписаниеПакета, Знач РежимУстановки) Экспорт
	...
	Для Каждого КаталогИсполняемогоФайла Из КаталогиИсполняемыхФайлов Цикл
		...
		Если Не Новый Файл(ПутьККонфигурационномуФайлу).Существует() Тогда
			Лог.Отладка("Создан файл настройки запуска - %1", ПутьККонфигурационномуФайлу);
		...
		Иначе
			Лог.Отладка("Существующий файл настройки запуска не изменен - %1", ПутьККонфигурационномуФайлу);
		КонецЕсли;
