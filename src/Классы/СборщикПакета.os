...
Процедура СобратьПакет(Знач КаталогИсходников, Знач ФайлМанифеста, Знач ВыходнойКаталог, Знач ДобавитьОтметкуГит) Экспорт
	...
	Попытка
		...
		Если ДобавитьОтметкуГит Тогда

			ВнешнийКонтекст = Новый Структура("Описание, КаталогРепозитория", Манифест, РабочийКаталог);
			ПутьКСкрипту = ОбъединитьПути(ТекущийСценарий().Каталог, "..", "gitmark.os");
			ЗагрузитьСценарий(ПутьКСкрипту, ВнешнийКонтекст);
