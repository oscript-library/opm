...
Процедура СобратьПакет(Знач КаталогИсходников, Знач ФайлМанифеста, Знач ВыходнойКаталог) Экспорт
	...
	Попытка
		...
		Иначе
			...
			Если Не ФайлВыхКаталога.Существует() Тогда
		...
		СИ.УстановитьПеременнуюСреды("os.package.name",СвойстваПакета.Имя);
		СИ.УстановитьПеременнуюСреды("os.package.version",СвойстваПакета.Версия);
