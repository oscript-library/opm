...
Процедура ВыполнитьКоманду(Знач КомандаПриложения) Экспорт
	...
	Если ПустаяСтрока(ТокенАвторизации) Тогда
		ТокенАвторизации = НастройкиOpm.ПолучитьНастройки().ТокенПубликацииПакетов;
	КонецЕсли;
...
Функция ПолучитьПараметрыСоединенияПубликации(Знач ИмяСервераПакетов) Экспорт
	...
	Результат.Вставить("ИспользоватьПрокси", Настройки.ИспользоватьПрокси);
...
Процедура ОтправитьПакетВХаб(Знач ТокенАвторизации, Знач ФайлПакета, Знач Канал, Знач ИмяСервераПакетов)
	...
	ПараметрыСоединения = ПолучитьПараметрыСоединенияПубликации(ИмяСервераПакетов);
	Сервер = ПараметрыСоединения.Сервер;
	Ресурс = ПараметрыСоединения.Ресурс;
	...
	Если ПараметрыСоединения.ИспользоватьПрокси Тогда
		Соединение = Новый HTTPСоединение(Сервер, Порт, , , ПараметрыСоединения.НастройкиПрокси);
	Иначе
