...
Процедура ОписаниеКоманды(Знач КомандаПриложения) Экспорт
	...
	ОпцияЗеркала = КомандаПриложения.Опция("m mirror", "", "УСТАРЕЛА. Указать имя сервера, с которого необходимо ставить пакеты.
					|    Доступные сервера прописываются в конфигурационном файле opm.cfg, параметр 'СервераПакетов'.
					|    Вместо неё указывайте адрес хаба прямо в аргументе: хост[:порт]/пул/пакет[@спецификация].")
	...
	ПараметрыПриложенияOpm.ОписатьОпцииПроверкиСуммы(КомандаПриложения);

	КомандаПриложения.Аргумент("PACKAGE", "", "Имя пакета в хабе. После @ можно указать спецификацию:
						|    точную версию (пакет@1.0.0), имя тега (пакет@stable) или выражение версии (пакет@^1.4).
						|    Тег и выражение разрешает хаб; хаб, не принимающий параметр spec, отвечает отказом.
						|    Можно указать адрес хаба целиком: [схема://]хост[:порт]/пул/пакет[@спецификация].")
						.ТМассивСтрок()
						.Обязательный(Ложь);

	// КомандаПриложения.Спек = "(-l | --local | -d | --dest )";
...
Процедура ВыполнитьКоманду(Знач КомандаПриложения) Экспорт
	...
	ПараметрыПриложенияOpm.ПрименитьОпцииПроверкиСуммы(КомандаПриложения);
	ПараметрыПриложенияOpm.ПредупредитьОбУстаревшемЗеркале(ИмяСервера);
	...
	Если ПустаяСтрока(ФайлПакетаУстановки) И ПустаяСтрока(ИнтернетСсылкаНаПакет) И МассивПакетовКУстановке.Количество() = 0 Тогда
	...
	Иначе
		...
		Для каждого ИмяПакета Из МассивПакетовКУстановке Цикл
			...
			ИначеЕсли АдресаХаба.ЭтоАдресХаба(ИмяПакета) Тогда

				РаботаСПакетами.УстановитьПакетПоАдресу(ИмяПакета, РежимУстановки, ЦелевойКаталог, НастройкаУстановки);
